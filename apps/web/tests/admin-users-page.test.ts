...
describe("buildUsersListViewModel", () => {
  ...
  it("returns the list kind with one row per user, preserving order and isActive", () => {
    const viewModel = buildUsersListViewModel([
      userFixture({ id: "u-1", email: "a@x.com", isActive: true }),
      userFixture({ id: "u-2", email: "b@x.com", isActive: false }),
    ]);
    ...
    if (viewModel.kind === "list") {
      ...
      assert.equal(viewModel.rows[0]!.isActive, true);
      assert.equal(viewModel.rows[1]!.id, "u-2");
      assert.equal(viewModel.rows[1]!.isActive, false);
    }
  });
...
describe("badge variant helpers", () => {
  it("maps ADMIN to a brand badge and STAFF to a neutral badge", () => {
    assert.equal(roleBadgeVariant("ADMIN"), "brand");
    assert.equal(roleBadgeVariant("STAFF"), "neutral");
  });

  it("maps active to success and inactive to danger", () => {
    assert.equal(statusBadgeVariant(true), "success");
    assert.equal(statusBadgeVariant(false), "danger");
  });
});
...
describe("renderUsersView composition", () => {
  ...
  it("wraps the UsersClient inside AppShell with admin-users context", () => {
    ...
    assert.equal(props.children.type, UsersClient);
  });
...
describe("UsersPage route", () => {
  ...
  it("renders AppShell with the UsersClient for ADMIN users", async () => {
  ...
  it("forwards the q search param into listUsers", async () => {
